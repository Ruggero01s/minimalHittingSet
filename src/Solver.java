...
public class Solver {
    ...
    public void solve(Instance instance) {
        ...
        do {
            ...
            next = next.reversed();
            current = next;
            if (!isInterrupted && !current.isEmpty())
                System.out.println("Completed level " + instance.getPerLevelHypotheses().size());
        }
        while (!current.isEmpty() && !isInterrupted);
    }

    /**
     * Interrupts the method at the next hypothesis
     */
