...
public class Main
{
	...
	static String fileName="74181.043.matrix";
		...
		do
		{
			...
			for (int i=0; i<current.size(); i++)
			{
				...
				else if (h.getBinaryRep().getFirst()!=1)
				{
					...
					int previousLength = current.size();
					removeAllBiggerHypothesis(current, h2);
					int postLength=current.size();
					i -=(previousLength-postLength);
   ...
   /* private static List<Hypothesis> calculateMHS() {
	...
	{
        current.removeIf(h -> h.isGreater(h2));
	}
